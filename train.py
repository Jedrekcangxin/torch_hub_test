...
function : 
        No external data sets will be used to train the model.
        The model will generate its own data points.
"""
...
def main():
    ...
    # generate a 3-class classification problem with 1000 data points, where each data point is a 4D feature vector
    ...
    _mlp = mlp.get_training_model().to(DEVICE)
    print(_mlp)

    # initialize optimizer and loss function
    opt = SGD(_mlp.parameters(), lr=LR)
    ...
    for epoch in range(0, EPOCHS):
        ...
        for (batchX, batchY) in next_batch(trainX, trainY, BATCH_SIZE):
            ...
            predictions = _mlp(batchX)
            loss = lossFunc(predictions, batchY.long())

            # zero the gradients accumulated from the previous steps, perform backpropagation, and update model parameters
            opt.zero_grad()
            loss.backward()
            opt.step()
    ...
    torch.save(_mlp.state_dict(), MODEL_PATH)
